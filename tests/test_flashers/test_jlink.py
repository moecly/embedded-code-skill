...
from tools.flashers.jlink import JLinkFlasher
...
class TestJLinkFlasher:
    ...
    def test_flash_hex(self, mocker):
        """测试 hex 文件烧录"""
    ...
    def test_flash_elf(self, mocker):
        """测试 elf 文件烧录"""
        mock_result = mocker.MagicMock()
        mock_result.stdout = b"Downloading file...O.K.\n"
        mock_result.stderr = b""
...
class TestJLinkFlasherHardware:
    """J-Link 硬件测试（需要实际连接设备）"""
    
    def test_jlink_flash_hex_hardware(self):
        """测试 J-Link 烧录 hex 文件（需要硬件）"""
        import tempfile
        
        flasher = JLinkFlasher()
        
        if not flasher.detect():
            pytest.skip("J-Link not connected, skipping hardware test")
    ...
    def test_jlink_flash_bin_hardware(self):
        """测试 J-Link 烧录 bin 文件（需要硬件）"""
        import tempfile
    ...
    def test_jlink_reset_hardware(self):
        """测试 J-Link 重置（需要硬件）"""
        flasher = JLinkFlasher()
